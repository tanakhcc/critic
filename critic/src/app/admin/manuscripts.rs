...
pub fn ManuscriptList() -> impl IntoView {
    ...
    view! {
        ...
        <div id="ms-sidebar-wrapper" class="flex flex-col justify-start w-1/4 overflow-auto border-r-2 border-slate-600">
            ...
            <div
                ...
                >
                <button class=DEFAULT_BUTTON_CLASSES
                    on:click=move |_| {
            ...
            <div id="new-manuscript-form"
                ...
                <form
                    ...
                    <input
                        ...
                        type="text"
                        node_ref=new_msname_ref name="msname"/>
                    <button
            ...
            <div
                ...
                <input id="ms-search" class="w-0 grow mr-2 ml-1 border-0 font-mono text-slate-400" type="search" name="msq" value=move || query.get()
                                ...
                                info.into_iter().map(|ms| {
                                        ...
                                        view!{
                                                ...
                                                if let Some(query_name) = query.get() {
                                                    Either::Left(
                                                        view!{
                                                            <a href=format!("/admin/manuscripts/{}?msq={}", ms.title, query_name)
                                                    ...
                                                    Either::Right(
                                                        view!{
                                                            <a href=format!("/admin/manuscripts/{}", ms.title)
                                                                class="w-0 grow my-2 bg-slate-600 p-2 text-center font-serif text-lg shadow-sm hover:bg-slate-500"
                                                                class=(["shadow-sky-600"], !is_selected())
                                                                class=(["shadow-slate-300", "text-sky-300"], is_selected())
                                                                >{ms.title.clone()}</a>
                    ...
                    match info_res {
                        ...
                        Ok(info) => {
                            ...
                            view!{
                                ...
                                <div id="manuscript-pageinfo-wrapper" class="flex justify-start min-h-96 max-h-full">
                                    // container for the left half of the lower half
                                    <div id="manuscript-pagelist-wrapper" class="flex h-full w-44 flex-col justify-start border-r-2 border-slate-600">
                                                ...
                                                info.pages.into_iter().map(|page| {
                                                    ...
                                                    <li class="flex">
                                                        <a
                                                            ...
                                                            class=(["shadow-slate-300", "text-sky-300"], is_selected())
                                                            class=(["shadow-sky-600"], !is_selected())
                                                            href=format!("/admin/manuscripts/{msname}/{}", page.name.clone())>{page.name.clone()}</a>
                                                        {
                                                            if let (Some(start), Some(end)) = (page.verse_start, page.verse_end) {
                                                                Some(view!{
                                                                    <p class="text-slate-300">{start}<span class="text-slate-500">-</span>{end}</p>
                                                                })
                                                            } else {
                                                                None
                                                            }
...
pub fn MMetaInput(
    ...
    view! {
          <div class=format!("grid grid-cols-2 border border-b-0 border-slate-500 p-2 {}", extra_class.unwrap_or_default())>
              <label for={name}>{children()}</label>
...
fn ManuscriptMeta(meta: critic_shared::ManuscriptMeta) -> impl IntoView {
    ...
    view! {
      <div class="p-6 border-2 border-slate-500">
        ...
        <h1 class="m-4 p-2 text-3xl text-center">"Manuscript "<span class="font-bold">{meta.title.clone()}</span></h1>
        <ActionForm
            action=srvact
            >
            <div
            ...
            <MMetaInput name="data[institution]" signal=institution extra_class="rounded-t-lg">Holding institution:</MMetaInput>
          ...
          <button class=format!("w-2/5 {DEFAULT_BUTTON_CLASSES}")
          type="button"
          on:click=move |_| {
              // reset the signals
                ...
                Ok(
                    view!{
                        <div class="flex w-0 flex-col grow justify-start">
                            ...
                            <img alt={format!("Preview for {msname} - {pagename}")} src={format!("{image_base}/preview.webp")} width=PREVIEW_IMAGE_WIDTH/>
                        </div>
                    }
                )
            } else {
