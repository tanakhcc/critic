...
use critic_shared::{
    urls::{IMAGE_BASE_LOCATION, MODEL_BASE_LOCATION},
    FileTransferResponse, ModelType, ALLOWED_IMAGE_EXTENSIONS, MAX_BODY_SIZE,
};
...
pub fn upload_router() -> axum::Router {
    axum::Router::new()
        ...
        .route(
            ...
            axum::routing::post(model_upload),
        )
...
async fn model_upload(
    Extension(config): Extension<Arc<Config>>,
    Path(model_type): Path<ModelType>,
    auth_session: AuthSession,
    mut mpart: Multipart,
) -> impl IntoResponse {
    let Some(user) = auth_session.user else {
    ...
    loop {
        match mpart.next_field().await {
            Ok(Some(field)) => {
                ...
                if extension.as_str() != "mlmodel" {
                    results.push_err("Extension must be .mlmodel.".to_string());
                    continue;
                };
                if dot_split.next().is_some() {
                    results.push_err("Filename did not contain exactly one dot.".to_string());
                ...
                // that worked - now deal with the file system
                let directory_path = format!(
                    "{}{}/{model_type}/{base_name}_{new_id}",
                    config.data_directory, MODEL_BASE_LOCATION,
                );
                if let Err(e) = std::fs::create_dir_all(&directory_path) {
                    results.push_err(format!(
                        "Failed to crate directory to put new page into: {e}."
                ...
                if let Err(e) = std::fs::write(format!("{directory_path}/original.mlmodel"), data) {
                    tracing::warn!("Unable to write model to file: {e}");
                    results.push_err("Failed to write model to file.".to_string());
                    continue;
                }
                tracing::info!(
                    "{} saved new model to {directory_path}/original.mlmodel.",
                    user.username
                );
                results.push_ok();
            }
