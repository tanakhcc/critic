...
pub(super) async fn load_editor_state() -> Result<Vec<EditorBlockDry>, ServerFnError> {
    ...
    let blocks: Vec<EditorBlockDry> = serde_json::from_reader(file).map_err(|e| ServerFnError::new(e.to_string()))?;
    Ok(blocks)
}
...
pub(super) async fn save_editor_state(blocks: Vec<EditorBlockDry>) -> Result<(), ServerFnError> {
    ...
    file.write_all(&serde_json::to_vec(&blocks).map_err(|e| ServerFnError::new(e.to_string()))?).map_err(|e| ServerFnError::new(e.to_string()))?;
    Ok(())
}
